...
from map.models import BusStop,RouteStops,Routes,LeaveTime,Trip,Vehicle
...
#         lastupdate = row[14]
#         note = row[15]
#         Trip.objects.get_or_create(DataSource=datasource, DayOfService=dayofservice, TripID=tripid,
#         LineID =Routes.objects.get(routeName=lineid),RouteID=routeid, Direction=direction, PlannedTime_Arr=plannedtime_arr,
...
# #
# # Check column order
# with open("rt_Vehicle.csv", "r") as f:
#     reader = csv.reader(f, delimiter=',')
#     next(reader, None)
#     for row in reader:
#         datasource = row[0]
...
#         lastupdate = row[5]
#         note = row[6]
#         Vehicle.objects.get_or_create(DataSource=datasource, DayOfService=dayofservice,
...
with open("rt_leavetimes_2017_I_DB_1000.txt", "r") as f:
    ...
    for row in reader:
        ...
        tripid = row[2]
        progrnumber = row[3]
        tripProg=tripid+ str(progrnumber)
        stoppointid = row[4]
        ...
        lastupdate = row[16]
        note = row[17]
        LeaveTime.objects.get_or_create(DataSource=datasource,DayOfService=dayofservice,TripID =Trip.objects.get(TripID=tripid),
        ProgrNumber=progrnumber,StopPointID=stoppointid,PlannedTime_Arr=plannedtime_arr,PlannedTime_Dep=plannedtime_dep,
                                     ActualTime_Arr=actualtime_arr,ActualTime_Dep=actualtime_dep,VehicleID=Vehicle.objects.get(VehicleID=vehicleid),
Passengers=passengers, PassengersIn=passengersin,PassengersOut=passengersout, Distance=distance,Suppressed =suppressed,
                                     JustificationID=justificationid,LastUpdate =lastupdate, Note=note,TripProg=tripProg)
# #